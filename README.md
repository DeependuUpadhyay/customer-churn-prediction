...
## 8. Deploying it live (for your resume link)

**Streamlit Community Cloud (free, easiest):**
1. Push this project to a public GitHub repo (make sure `models/` and
   `reports/` are generated locally first — either commit them, or add a
   `python run_pipeline.py` step to a `setup.sh` / GitHub Action so the
   cloud build trains the model on deploy).
2. Go to [share.streamlit.io](https://share.streamlit.io), sign in with
   GitHub, click "New app", pick the repo and set the main file to `app.py`.
3. You get a public URL like `https://your-app.streamlit.app` — put that
   directly on your resume/portfolio.

**Flask API (Render / Railway free tier):**
1. Push the same repo.
2. Create a new "Web Service" pointing at `api.py`, with the start command
   `gunicorn api:app` (add `gunicorn` to `requirements.txt` for production).
3. You get a public API URL for the `/predict` endpoint.

## 9. Suggested resume bullet points

Pick 2-3 depending on space — these are written to be truthful to exactly
what this project does:

- Built an end-to-end customer churn prediction system (Python,
  scikit-learn, XGBoost) achieving 0.85 ROC-AUC, engineering 8 features
  from raw account data and validating via stratified 5-fold cross-
  validated hyperparameter search across 3 model families.
- Handled severe class imbalance (26% positive rate) using SMOTE inside a
  leakage-safe cross-validation pipeline; evaluated business impact via
  lift and cumulative gains analysis showing a 2.9x improvement over
  random customer targeting.
- Deployed the model as an interactive Streamlit web application
  supporting single-customer lookup and batch CSV scoring, plus a Flask
  REST API for programmatic integration.

## 10. Possible extensions (mention these as "next steps" in an interview)

- Add SHAP values for per-prediction explainability in the app (currently
  the app shows rule-based risk flags; SHAP would give a true local
  explanation per customer).
- Track model performance over time / retrain on a schedule (data drift
  monitoring) since a real telecom's customer mix shifts.
- A/B test the retention offers triggered by high-risk scores to measure
  actual causal impact, not just predictive accuracy.
- Swap the point-in-time snapshot for a time-series formulation (predict
  churn in the next 30/60/90 days from a rolling feature window).
